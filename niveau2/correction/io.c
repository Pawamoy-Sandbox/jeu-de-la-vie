...
extern int (*compte_voisins_vivants) (int, int, grille); // CORRECTION
/** \brief défini dans jeu.c*/
extern void (*set_continue_vie) (int, int, grille); // CORRECTION
/** \brief décompte du temps qui passe : variable globale statique : n'est connue que dans io.c*/
static int temps = 0; // CORRECTION
...
void affiche_ligne (int c, int* ligne){
	int i;
	for (i=0; i<c; ++i) 
		if (ligne[i] == 0 ) printf ("|   "); else printf ("| %d ",ligne[i]); // CORRECTION
	printf("|\n");
	return;
}

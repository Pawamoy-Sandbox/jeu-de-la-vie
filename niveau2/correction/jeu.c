 ...
 */
int (*compte_voisins_vivants) (int, int, grille) = compte_voisins_non_cyclique ; // CORRECTION
 ...
 */
void (*set_continue_vie)(int,int,grille) = set_ne_viellit_pas ; // CORRECTION
...
int compte_voisins_non_cyclique (int i, int j, grille g){ // CORRECTION
	int v = 0, l=g.nbl, c = g.nbc;
	...
	for (i=0; i<l; i++)
	{
		for (j=0; j<c; ++j)
		{
			...
			{ // evolution d'une cellule vivante
				...
				else set_continue_vie(i,j,*g); // CORRECTION
			}
			else 
